...
A personal static blog built on **Astro 7** with **Svelte 5** for interactive components, forked from the [Firefly](https://github.com/CuteLeaf/Firefly) theme (itself a fork of [Fuwari](https://github.com/saicaca/fuwari)). Site identity lives in `src/config/siteConfig.ts`. Primary language is Chinese (Simplified) with i18n for en, zh_TW, ja, ko, ru.
...
| `pnpm build` | Production build (GitHub cards → LQIPs → VNDB covers → Astro build → pio asset pruning → font subsetting → inline-script minification → Pagefind indexing) |
...
| `pnpm lint` | Biome lint + auto-fix over `./src` and `./scripts` |
| `pnpm format` | Biome format over `./src` and `./scripts` |
...
| `pnpm github-cards` | Regenerate GitHub repo card data into `src/constants/github-card-data.json` |

Package manager is **pnpm** (enforced via `only-allow`). Node.js >= 22.23.0 required.
...
- `src/plugins/` — 19 custom remark/rehype plugins (Mermaid, PlantUML, KaTeX, GitHub cards, reading time, wiki links, etc.)
...
- `pnpm lint`/`pnpm format` target both `./src` and `./scripts`; CI (`.github/workflows/biome.yml`) still runs `biome ci ./src` only, so `scripts/` findings do not fail CI
...
LQIP data is generated into `src/constants/lqips.json` and committed — regenerate with `pnpm lqips`. GitHub repo card data is generated into `src/constants/github-card-data.json` and committed — regenerate with `pnpm github-cards` (it scans post sources for `::github{repo="..."}` directives). Icon data lives in `src/constants/icons-data.json` (committed, Biome-ignored, consumed by `src/components/common/Icon.svelte`) but has no generator script in the current build.
